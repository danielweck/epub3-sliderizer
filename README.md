...
EPUB3-Sliderizer is based on the promise of "single file editing", but unlike other HTML slideshow frameworks, EPUB3-Sliderizer keeps each slide of a deck as a separate XHTML file. This way, the files can be used as "pages" in the context of a pre-paginated e-book (aka "fixed layout"). Furthermore, the actual content can be authored with Markdown, using tag-soup HTML, or with clean XHTML. Your choice.

When an EPUB3 file generated by EPUB3-Sliderizer is unzipped, each HTML file is capable of "self-rendering" within a web browser (using its own built-in navigation user interface, or "chrome"), delivering a rich viewing experience on both mobile "touch" devices and keyboard-driven desktops.
