...
var Epub3Sliderizer = {
    ...
    opera: window.opera || navigator.userAgent.toLowerCase().indexOf(' opr/') >= 0,
...
Epub3Sliderizer.gotoPrevious = function()
{
    ...
    if (navigator.epubReadingSystem
        ...
        && window.READIUM_activeEvents[navigator.epubReadingSystem.Pagination.EVENT_PAGE_PREVIOUS]
        && window.READIUM_activeEvents[navigator.epubReadingSystem.Pagination.EVENT_PAGE_PREVIOUS].length)
    {
        window.postMessage({event: navigator.epubReadingSystem.Pagination.EVENT_PAGE_PREVIOUS}, "*");
        return;
    }
...
Epub3Sliderizer.spinner = function(previous)
{
    var spinner = document.getElementById("slideSpinner");
    
    var dim = Math.min(Math.floor(window.innerWidth / 5), Math.floor(window.innerHeight / 5));
    
    spinner.style.height = dim + "px";
    spinner.style.width = dim + "px";
    
    spinner.style.top = Math.floor(window.innerHeight / 2 - dim / 2) + "px";
    
    if (previous)
    {
        spinner.style.left = dim + "px";
    }
    else
    {
        spinner.style.left = "auto";
        spinner.style.right = dim + "px";
    }
...
Epub3Sliderizer.gotoPreviousGo = function()
{
    ...
    setTimeout(function(){
        window.location = that.prev + that.urlParams(true);
    }, 100);
};
...
Epub3Sliderizer.gotoNext = function()
{
    ...
    if (navigator.epubReadingSystem
        && navigator.epubReadingSystem.Pagination
        && window.READIUM_activeEvents
        && window.READIUM_activeEvents[navigator.epubReadingSystem.Pagination.EVENT_PAGE_NEXT]
        && window.READIUM_activeEvents[navigator.epubReadingSystem.Pagination.EVENT_PAGE_NEXT].length)
    {
        window.postMessage({event: navigator.epubReadingSystem.Pagination.EVENT_PAGE_NEXT}, "*");
        return;
    }
...
Epub3Sliderizer.gotoNextGo = function()
{
    ...
    setTimeout(function(){
        window.location = that.next + that.urlParams(true);
    }, 100);
};
...
Epub3Sliderizer.fetchLocalStorageTextArea = function()
{
    if (this.authorMode)
    {
        ...
        loadScript(undefined, 'jquery.min.js');
    }
    ...
    if (hasLocalStorage())
    {
        var thisKey = window.location.pathname ? window.location.pathname : this.thisFilename;
        console.log(":::: LOCALSTORAGE KEY (FETCHING...): " + thisKey);
        ...
        if (data)
        {
            ...
            var tryAgain_$ = function()
            {
                if (!window.$)
                {
                    setTimeout(tryAgain_$, 100);
                    return;
                }
                ...
                that.updateSlideContent(data); // AUTHORize() done in there
            };
            tryAgain_$();
...
Epub3Sliderizer.updateLocalStorageTextArea = function(content)
{
    ...
    {
        var thisKey = window.location.pathname ? window.location.pathname : this.thisFilename;
        console.log(":::: LOCALSTORAGE KEY (UPDATING...): " + thisKey);
        ...
        localStorage.setItem(thisKey, content);
...
//            this.convertContentToMarkdownAndUpdateLocalStorageTextArea_Throttled();
    ...
    if (!txtArea)
    {
        alert('Author mode is not active for this slide.\n\nNote that this mode is disabled by default with XML slides (XHTML),\nunless content field is explicitly set to MARKDOWN.\n\n Try the HTML slide instead?\n(just change the file extension in the address bar from .xhtml to .html).\n\n[' + this.thisFilename + "]");
        return;
    }
...
Epub3Sliderizer.initTouch_ = function(onTap)
{
    ...
    var onDragStart_ = function(hammerEvent)
    {
        ...
        if (hammerEvent.gesture)
        {
console.debug("EPSLIDRZ HAMMER PREVENT DEFAULT");
            hammerEvent.gesture.preventDefault();
            //hammerEvent.gesture.stopPropagation();
        }
        else
        {
console.debug("EPSLIDRZ HAMMER PREVENT DEFAULT ALT?");
...
console.debug("EPSLIDRZ SWIPE (LEFT)");
        ...
        {
console.debug("EPSLIDRZ navigator.epubReadingSystem.Pagination.TouchSuspended (NEXT)");
            return;
        }
...
console.debug("EPSLIDRZ SWIPE (RIGHT)");
        if (navigator.epubReadingSystem && navigator.epubReadingSystem.Pagination && navigator.epubReadingSystem.Pagination.TouchSuspend())
        {
console.debug("EPSLIDRZ navigator.epubReadingSystem.Pagination.TouchSuspended (PREVIOUS)");
            return;
        }
    ...
    )
    {
        ...
        //hammerEvent.gesture.preventDefault();
console.debug("EPSLIDRZ SWIPE (UP)");
        if (navigator.epubReadingSystem && navigator.epubReadingSystem.Pagination && navigator.epubReadingSystem.Pagination.TouchSuspend())
        {
console.debug("EPSLIDRZ navigator.epubReadingSystem.Pagination.TouchSuspended (UP)");
            return;
        }
    ...
    )
    {
        ...
        //hammerEvent.gesture.preventDefault();
console.debug("EPSLIDRZ SWIPE (DOWN)");
        if (navigator.epubReadingSystem && navigator.epubReadingSystem.Pagination && navigator.epubReadingSystem.Pagination.TouchSuspend())
        {
console.debug("EPSLIDRZ navigator.epubReadingSystem.Pagination.TouchSuspended (DOWN)");
            return;
        }
...
{
    if (!navigator.epubReadingSystem || !navigator.epubReadingSystem.Pagination) return;
    ...
    {
        navigator.epubReadingSystem.Pagination.Direction = this.reverse ? navigator.epubReadingSystem.Pagination.EVENT_PAGE_PREVIOUS : navigator.epubReadingSystem.Pagination.EVENT_PAGE_NEXT;
    }
}
...
//logHTML("LATE");
    ...
    else if (canTryParent && window.parent.navigator.epubReadingSystem)
    {
//logHTML("LATE PARENT");
...
{   
    ...
    this.kobo = window.KOBO_TAG; // window.nextKoboSpan
...
//    this.convertContentToMarkdownAndUpdateLocalStorageTextArea_Throttled = throttle(this.convertContentToMarkdownAndUpdateLocalStorageTextArea, 1000, false).bind(this);
    ...
    {
        this.fetchLocalStorageTextArea();
        
        if (!this.mobile)
        {
            var needKeymaster = false;
            if (hasLocalStorage())
            {
                var thisKey = window.location.pathname ? window.location.pathname : this.thisFilename;
            ...
            if (this.authorMode && Epub3Sliderizer.htmlNotXHTML
                || needKeymaster)
            {
                loadScript(undefined, 'keymaster.js');
                var tryAgain_Key = function()
                {
                    if (!window.key)
                    {
                        setTimeout(tryAgain_Key, 100);
                        return;
                    }
            
                    if (that.authorMode && Epub3Sliderizer.htmlNotXHTML)
                    {
                    ...
                    key('⌘+d, ctrl+d', function()
                    {
                        if (hasLocalStorage())
                        {
                            var thisKey = window.location.pathname ? window.location.pathname : that.thisFilename;
                            console.log(":::: LOCALSTORAGE KEY (DISCARDING...): " + thisKey);
            
                            for (var i = 0; i < localStorage.length; i++)
                            {
                                var key = localStorage.key(i);
                                if (key === thisKey
                                    && confirm("Discard changes to [" + that.thisFilename + "] ?"))
                                {
                                    //var data = localStorage.getItem(key);
                                    localStorage.removeItem(key);
...
function readyFirst()
{
    Epub3Sliderizer.readium = window.LauncherUI || window.ReadiumSDK || window.Readium;
    if (!Epub3Sliderizer.readium)
    {
        ...
        else
        {
            try
            {
                Epub3Sliderizer.readium = window.parent.ReadiumSDK;
            }
            catch(e)
            {
                console.error(e);
            ...
            if (!Epub3Sliderizer.readium)
            {
                try
                {
                    Epub3Sliderizer.readium = window.parent.Readium;
                }
                catch(e)
                {
                    console.error(e);
    ...
    Epub3Sliderizer.ibooks = window.iBooks || iBooksX;
    Epub3Sliderizer.playbooks = window.editions;
